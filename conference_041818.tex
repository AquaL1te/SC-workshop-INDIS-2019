...
\begin{enumerate}
    \item Fault tolerance challenges. SeaDataNet originally only gathers metadata from remote repositories, and the user has to access the individual remote repositories\footnote{We distinguish repository and registry based on the Digital Object Architecture: registry is for storing metadata while the repository is for storing the data objects. They can often be combined\cite{berg_cross_gary_2015_2574407}. } to obtain data. Such client-server style not only creates high load for the repository but also network traffic congestion.
